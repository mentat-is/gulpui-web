...
import { bich, copy, fws } from '@/ui/utils';
...
export namespace OpenSearchQueryBuilder {
  export type Condition = 'term' | 'match' | 'regexp' | 'prefix' | 'wildcard' | 'range';
...
export function FilterFileBanner({ file, ...props }: FilterFileBannerProps) {
  ...
  const addCondition = () => Info.setFilters(file.id, [...query.filters, {
  ...
  const Done = () => <Button img='Check' variant='glass' loading={loading} onClick={submit} />;
  const Undo = () => <Button img='Undo' variant='ghost' onClick={() => Info.filters_undo(file)} />;
  ...
  const QueryStringPart = useMemo(() => {
    return (
      <Stack dir='column' gap={6} style={fws} ai='stretch'>
        <p>Query String</p>
        <Stack>
          <Input variant='highlighted' img='Code' placeholder='Enter query_string part...' value={query.string} onChange={e => Info.setQueryString(file.id, e.target.value)} />
          <Button img='Copy' variant='secondary' onClick={() => copy(query.string)} />
          <Button img='Undo2' variant='secondary' onClick={() => Info.setQueryString(file.id, Filter.base(file))} revert>Reset</Button>
        </Stack>
      </Stack> 
    )
  }, [file, Info]);
  ...
  const Preview = () => {
    ...
    return (
      <Stack pos='relative' className={s.preview}>
        <Button className={s.copy} img='Copy' onClick={() => copy(string)} variant='glass' />
  ...
  const removeCondition = useCallback((id: string) => {
    Info.setFilters(file.id, query.filters.filter((condition) => condition.id !== id))
  }, [file.id, Info.setFilters]);

  const updateCondition = useCallback((id: string, field: string, value: string) => {
    Info.setFilters(file.id, query.filters.map((condition) => (condition.id === id ? { ...condition, [field]: value } : condition)))
  }, [file.id, Info.setFilters]);
    ...
    return (
      <Stack ai='stretch' dir='column'>
        {query.filters.map((condition) => (     
          <Stack dir='column' ai='flex-start' className={s.card} key={condition.id}>
            <Stack style={fws}>
            ...
            <Stack style={fws}>
              <Select value={condition.field} onValueChange={e => updateCondition(condition.id, 'field', e)}>
  ...
  return (
    <Banner
      ...
      <Stack style={fws} jc='space-between'>
        ...
        <Button onClick={addCondition} variant='secondary' img='Plus'>Add condition</Button>
      </Stack>
      <Separator />
      {QueryConditions}
