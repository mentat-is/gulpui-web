...
import { APIProvider, Map as VisMap, GoogleMapsContext, latLngEquals } from '@vis.gl/react-google-maps';
import { useState, forwardRef, useContext, useEffect, useImperativeHandle, useRef } from 'react';
...
export namespace Maps {
  ...
  export const Banner = ({ ip, lat, lng }: Maps.Props) => {
    ...
    const ToggleFullscreen = () => {
      return (
        <Button variant='glass' img={fullscreen ? 'Minimize' : 'Maximize'} onClick={() => setFullscreen(v => !v)} />
      )
    }

    return (
      <UIBanner title={ip ? `Location of ${ip}` : 'Maps'} className={cn(s.banner, fullscreen && s.fullscreen)} done={<ToggleFullscreen />}>
        <APIProvider apiKey={'AIzaSyDhBmfhciBYS9xUAifZaCQTJu4tVYr16TQ'}>
          <VisMap
            ...
            disableDefaultUI
          >
            <Circle.Element center={{ lat, lng }} radius={18000} fillColor={'#3b82f6 '} strokeColor={'#0c4cb3'} strokeOpacity={1} strokeWeight={3} fillOpacity={0.3} />
          </VisMap>
  ...
  export const Element = forwardRef((props: Circle.Props, ref: Circle.Reference) => {
