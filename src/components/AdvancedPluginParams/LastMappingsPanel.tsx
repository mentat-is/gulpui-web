...
export function LastMappingsPanel({
  ...
  const [previewMapping, setPreviewMapping] = useState<SavedPluginMapping | null>(null)
  ...
  const getSavedMappingSummary = (savedMapping: SavedPluginMapping): string => {
    return savedMapping.mapping.description || savedMapping.mapping.agent_type || 'Custom mapping'
  }
  ...
  return (
    <>
        ...
        <Dialog.Portal>
          ...
          >
            <Stack dir="column" gap={16} ai="stretch">
              ...
              <Stack dir="column" gap={8} ai="stretch" className={s.savedMappingsList}>
                {loading && (
                  <Label value="Loading mappings..." className={s.savedMappingsEmpty} />
                )}

                {!loading && savedMappings.length === 0 && (
                  <Label value="No saved mappings" className={s.savedMappingsEmpty} />
                )}

                {!loading && savedMappings.map((savedMapping) => (
                  <div
                    ...
                    onKeyDown={(event) => handleMappingRowKeyDown(event, savedMapping)}
                  >
                    <Stack dir="column" gap={4} ai="stretch" className={s.savedMappingDetails}>
                      <span className={s.savedMappingTitle}>{savedMapping.mapping_id}</span>
                      <span className={s.savedMappingMeta}>{getSavedMappingSummary(savedMapping)}</span>
                      <span className={s.savedMappingDate}>{formatSavedMappingDate(savedMapping.updated_at)}</span>
                    </Stack>

                    <Stack dir="row" gap={4} ai="center" className={s.savedMappingActions}>
                      <Button
                        variant="glass"
                        icon="MagnifyingGlass"
                        shape="icon"
                        aria-label={`Preview ${savedMapping.mapping_id}`}
                        onClick={(event) => handlePreview(event, savedMapping)}
                      />
                      <Button
                        variant="tertiary"
                        icon="Trash"
              ...
              <Stack dir="row" gap={8} jc="flex-end">
      ...
      <Dialog.Root open={!!previewMapping} onOpenChange={(isOpen) => !isOpen && setPreviewMapping(null)}>
        <Dialog.Portal>
          ...
          >
            <Stack dir="column" gap={16} ai="stretch">
              <Dialog.Title className={cn(s.titleBase, s.titleM)}>
                {previewMapping ? `Preview ${previewMapping.mapping_id}` : 'Preview Mapping'}
              </Dialog.Title>
              <pre className={s.mappingPreviewCode}>
                {previewMapping ? JSON.stringify(previewMapping.mapping, null, 2) : ''}
              </pre>
              <Stack dir="row" gap={8} jc="flex-end">
                <Button variant="secondary" onClick={() => setPreviewMapping(null)}>
                  Close
                </Button>
              </Stack>
            </Stack>
