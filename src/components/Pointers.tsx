...
import { stringToHexColor, λColor } from "@/ui/utils";
...
export function Pointers({ getPixelPosition, scrollY, width, self, timestamp, ...props }: Pointers.Props) {
  ...
  return (
    <Stack pos='absolute' className={s.pointers}>
      {[you, ...app.timeline.pointers].map(p => {
        ...
        return (
          <Stack className={cn(s.pointer, isRightSide && s.right)} style={{ top: isYours ? p.y : (-scrollY + p.y), left: x }} pos='absolute'>
            <Icon name='Pointer' color={p.color} fill={p.color} />
            <p style={{ background: p.color }}>{p.id} {isYours ? `on ${getDate(timestamp)}ms` : null}</p>
          </Stack>
        )
      })}
    </Stack>
