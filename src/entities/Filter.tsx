...
export namespace Filter {
	...
	export class Entity {
		static query = (
			q: Query.Type & { fieldTypeMap?: Record<string, string> },
		) => {
		...
		public static base = (files: Arrayed<Source.Type>, range?: MinMax) =>
			Parser.array(files)
				.map(
					(file) =>
						`(gulp.operation_id: ${Filter.Entity.quotes(file.operation_id)} AND gulp.context_id: "${Filter.Entity.quotes(file.context_id)}" AND gulp.source_id: "${Filter.Entity.quotes(file.id)}" AND gulp.timestamp: [${range?.min ?? file.nanotimestamp?.min ?? Internal.Transformator.toNanos(file.timestamp.min)} TO ${range?.max ?? file.nanotimestamp?.max ?? Internal.Transformator.toNanos(file.timestamp.max)}])`,
				)
