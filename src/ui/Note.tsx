...
export namespace NotePoint {
  export interface Props extends Omit<UIPoint.Props, 'icon' | 'accent' | 'name'> {
    note: λNote;
  }
  ...
  export function Combination({ className, note, ...props }: Combination.Props) {
    ...
    return (
      <Stack className={cn(s.combination, className)} style={{ color: note.color }} {...props}>
        <Icon name={Note.icon(note)}  />
        <p>{note.name}</p>
        <span>{note.description}</span>
        <Button img='MagnifyingGlassSmall' onClick={() => targetNoteButtonHandler(note)} variant='ghost' />
      </Stack>
    )
  }
  ...
  export function Point({ note, ...props }: NotePoint.Props) {
    ...
    return (
      <UIPoint onClick={openEvent} icon={Note.icon(note)} accent={note.color} name={note.name} {...props} />
    )
  }
